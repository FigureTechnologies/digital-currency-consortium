...
class DigitalCurrencyController(
    ...
    fun grantAllowance(
        ...
        pbcService.getGrant()?.also {
            log.info("Replacing existing grant: $it")
            pbcService.revokeGrant()
        }

        val coins = request.coins.map {
            Coin.newBuilder()
                .setDenom(it.denom)
                .setAmount(it.quantity.toString())
                .build()
        }
        pbcService.grantBasicAllowance(coins)
