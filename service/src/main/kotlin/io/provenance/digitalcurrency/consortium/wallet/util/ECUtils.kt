...
object ECUtils {
    ...
    val KNOWN_CURVES = arrayOf(
        "secp128r1", "secp160k1", "secp160r1", "secp160r2", "secp192k1",
    ...
    fun privateKeyFromPaddedByteArray(base64EncodedPaddedPrivateKey: String): PrivateKey {
        val privateKeyInteger = unsignedBytesToBigInt(BaseEncoding.base64().decode(base64EncodedPaddedPrivateKey))
    ...
    fun toBigInt(value: ByteArray, offset: Int, length: Int): BigInteger {
        return unsignedBytesToBigInt(Arrays.copyOfRange(value, offset, offset + length))
    }

    fun toBigInt(value: ByteArray): BigInteger {
        return unsignedBytesToBigInt(value)
    }
    ...
    //     val publicKey: PublicKey = convertBytesToPublicKey(decodeString(base64EncodedPubKey), curve)
    //
    //     return ProvenanceECIESCryptogram(
    //         publicKey,
    //         decodeString(base64EncodedTag),
    //         decodeString(base64EncodedEncryptedData)
    //     )
    // }
    //
    ...
    fun publicKeyEncoded(publicKey: PublicKey): String {
        return Base64.getEncoder().encode(convertPublicKeyToBytes(publicKey)).toString(Charsets.UTF_8)
    }
