...
open class TxRequestViewEntityClass : UUIDEntityClass<TxRequestViewRecord>(TRV) {
    ...
    fun findExpiredWithTxHash(blockHeight: Long) = find {
        (TRV.updated lessEq OffsetDateTime.now().minusSeconds(30))
            .and(TRV.status eq TxStatus.PENDING)
            .and(TRV.txHash.isNotNull())
            .and(TRV.timeoutHeight.isNull() or (TRV.timeoutHeight less blockHeight))
    }
}
